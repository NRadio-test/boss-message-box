...
import { ImageValidationError, validatePrivateWebpBytes } from "../security/image";
...
export class CloudflareImageProcessor implements ImageProcessor {
  ...
  async sanitize(file: File): Promise<ProcessedImage> {
    if (file.size <= 0) throw new ImageValidationError("图片内容无效，请重新选择");
    const info = await this.images.info(file.stream());
    if (
    ...
    const output = await this.encode(file, info.width, info.height, TARGET_EDGE, 84);
    return validatePrivateWebpBytes(output);
  }
