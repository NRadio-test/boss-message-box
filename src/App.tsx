  ...
  if (error) {
    return (
      <div className="boot-state" role="alert">
        <h1>暂时无法打开留言箱</h1>
        <p>{error}</p>
        <Button icon={<ArrowClockwise aria-hidden="true" />} onClick={() => setReload((value) => value + 1)}>重新连接</Button>
      </div>
    );
  }
  if (!config) {
    return <div className="boot-state" aria-live="polite"><span className="boot-signal" /><p>正在打开留言箱…</p></div>;
  }
