...
function PrivacyContent({ version }: { version: string }) {
  return (
    <>
      ...
      <h3>我们收集哪些信息</h3>
      <p>我们会收集你的抖音昵称、留言主题与内容、你主动上传的图片、两项同意记录，以及保障服务安全所必需的网络和操作日志。当前版本不收集手机号，也不进行短信验证。未提交的文字与图片草稿仅保存在你的浏览器中。</p>
      <h3>这些信息如何使用</h3>
      <p>昵称用于每日提交次数统计和留言查询；留言及图片用于问题处理、产品改进和直播回复。昵称查询不是身份验证，同名用户可以查看该昵称下的留言和回复，请勿提交敏感信息。</p>
      <h3>直播与公开展示</h3>
      <p>正式提交后，你的抖音昵称、留言文字和提交内容可能在公司直播中被公开展示和回复。</p>
      <h3>服务提供方</h3>
      <p>为提供本服务，我们使用 Cloudflare 的计算、数据库、对象存储、图片处理和安全能力。提交后，留言主题与正文还会交由配置的 AI 服务提供方进行内容筛选；此筛选请求不包含你的昵称、图片或其他身份信息。AI 判断可能有误，工作人员可以复核和恢复留言。上述服务方在提供服务所必需的范围内处理信息。</p>
      <h3>图片与安全</h3>
...
function LivestreamContent() {
  return (
    <>
      <p>正式提交的留言会进入内容筛选和工作人员处理流程，不保证每条留言都在直播中展示或回复。</p>
      <p>你提交的<strong>抖音昵称、留言文字、留言主题和图片等内容</strong>，可能在鲲鹏无限科技有限公司的直播中公开展示，并由主播或工作人员现场回复。</p>
      <p>请不要在留言或图片中放入身份证号、住址、银行卡、私人联系方式等不希望公开的信息。</p>
    </>
  );
}
